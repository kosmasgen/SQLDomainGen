...
public class EntityParser {
    ...
    public List<Entity> parseEntities(String entitiesDir) {
        log.info("🔍 Ξεκινάει η σάρωση του φακέλου: {}", entitiesDir);
        List<Entity> entityList = new ArrayList<>();

        try (DirectoryStream<Path> stream = Files.newDirectoryStream(Paths.get(entitiesDir), "*.java")) {
            for (Path entry : stream) {
                log.info("📄 Διαβάζεται το αρχείο entity: {}", entry.getFileName());
                Entity entity = parseEntityFile(entry);
                if (entity != null) {
                    entityList.add(entity);
                    log.info("✅ Προστέθηκε η οντότητα: {}", entity.getName());
                } else {
                    log.warn("⚠️ Δεν δημιουργήθηκε οντότητα από το αρχείο: {}", entry.getFileName());
                }
            }
        } catch (IOException e) {
            log.error("❌ Σφάλμα κατά την ανάγνωση του φακέλου: {}", entitiesDir, e);
        }

        log.info("🏁 Ολοκληρώθηκε η ανάγνωση των οντοτήτων. Σύνολο: {}", entityList.size());
        return entityList;
    }

    private Entity parseEntityFile(Path entityFilePath) {
        log.debug("📄 Ξεκινάει η ανάγνωση του αρχείου: {}", entityFilePath);
        try {
            List<String> lines = Files.readAllLines(entityFilePath);
            Entity entity = extractEntityInfo(lines);
            if (entity != null) {
                log.info("✅ Επιτυχής εξαγωγή οντότητας από το αρχείο: {} -> {}", entityFilePath.getFileName(), entity.getName());
            } else {
                log.warn("⚠️ Αποτυχία εξαγωγής οντότητας από το αρχείο: {}", entityFilePath.getFileName());
            }
            return entity;
        } catch (IOException e) {
            log.error("❌ Σφάλμα ανάγνωσης αρχείου: {}", entityFilePath, e);
            return null;
        }
    }

    private Entity extractEntityInfo(List<String> lines) {
        log.debug("🔍 Ξεκινά η επεξεργασία γραμμών για εξαγωγή οντότητας...");
        ...
        for (String line : lines) {
            ...
            if (line.startsWith("private ")) {
                ...
                log.debug("🆕 Εντοπίστηκε πεδίο: {} τύπου: {}", field.getName(), field.getType());

                processAnnotations(currentAnnotations, field);
                fields.add(field);

                log.info("🔍 Πεδίο -> Όνομα: {}, Τύπος: {}, PrimaryKey: {}, ForeignKey: {}, Nullable: {}, Unique: {}, Length: {}, ColumnName: {}, ReferencedEntity: {}, ReferencedColumn: {}",
        ...
        if (className == null) {
            log.warn("⚠️ Δεν βρέθηκε κλάση στην οντότητα!");
            return null;
        }

        log.info("✅ Η οντότητα '{}' εξάχθηκε με {} πεδία.", className, fields.size());
    ...
    private String extractClassName(String line) {
        ...
        if (matcher.find()) {
            ...
            log.debug("🔹 Εντοπίστηκε κλάση: {}", className);
            return className;
        }
        return null;
    }
    ...
    private Field extractField(String line) {
        ...
        log.debug("🏷️ Δημιουργήθηκε πεδίο -> Όνομα: {}, Τύπος: {}", name, type);
        return field;
    }

    private void processAnnotations(List<String> annotations, Field field) {
        log.debug("📜 Επεξεργασία annotations για το πεδίο: {} -> {}", field.getName(), annotations);
    ...
    private void processRelationshipAnnotation(Field field) {
        ...
        log.debug("🔗 Foreign Key -> {} -> {}", field.getName(), field.getReferencedEntity());
    }
    ...
    private void processColumnAnnotation(String annotation, Field field) {
        log.debug("🏷️ Επεξεργασία @Column για το πεδίο: {}", field.getName());
    ...
    private void processJoinColumnAnnotation(String annotation, Field field) {
        log.debug("🔗 Επεξεργασία @JoinColumn για το πεδίο: {}", field.getName());
    ...
    public Entity parseEntityFromFile(Path entityFilePath) {
        log.info("📄 Έναρξη parsing για μεμονωμένο αρχείο: {}", entityFilePath);
