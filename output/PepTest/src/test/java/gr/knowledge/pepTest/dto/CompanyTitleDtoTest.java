package gr.knowledge.pepTest.dto;

import org.junit.jupiter.api.Test;
import static org.assertj.core.api.Assertions.assertThat;
import java.util.*;
import java.math.*;
import java.time.*;
import java.util.UUID;
import java.math.BigInteger;
import java.time.LocalDateTime;

class CompanyTitleDtoTest {

    /**
     * Tests the CompanyTitleDto no-args constructor
     */
    @Test
    void testCompanyTitleDtoNoArgsConstructor() {
        CompanyTitleDto dto = new CompanyTitleDto();

        assertThat(dto.getId()).isNull();
        assertThat(dto.getChamberId()).isNull();
        assertThat(dto.getTitle()).isNull();
        assertThat(dto.getChamberTitleId()).isNull();
        assertThat(dto.getCompany()).isNull();
        assertThat(dto.getDateCreated()).isNull();
        assertThat(dto.getLastUpdated()).isNull();
        assertThat(dto.getRecdeleted()).isNull();
    }

    /**
     * Tests the CompanyTitleDto all-args constructor
     */
    @Test
    void testCompanyTitleDtoAllArgsConstructor() {
        UUID id = UUID.fromString("123e4567-e89b-12d3-a456-426614174000");
        Integer chamberId = 1;
        String title = "test-value";
        BigInteger chamberTitleId = new BigInteger("1");
        CompanyDto company = new CompanyDto();
        LocalDateTime dateCreated = LocalDateTime.of(2025, 1, 1, 10, 0);
        LocalDateTime lastUpdated = LocalDateTime.of(2025, 1, 1, 10, 0);
        Boolean recdeleted = Boolean.TRUE;

        CompanyTitleDto dto = new CompanyTitleDto(id, chamberId, title, chamberTitleId, company, dateCreated, lastUpdated, recdeleted);

        assertThat(dto.getId()).isEqualTo(id);
        assertThat(dto.getChamberId()).isEqualTo(chamberId);
        assertThat(dto.getTitle()).isEqualTo(title);
        assertThat(dto.getChamberTitleId()).isEqualTo(chamberTitleId);
        assertThat(dto.getCompany()).isEqualTo(company);
        assertThat(dto.getDateCreated()).isEqualTo(dateCreated);
        assertThat(dto.getLastUpdated()).isEqualTo(lastUpdated);
        assertThat(dto.getRecdeleted()).isEqualTo(recdeleted);
    }

    /**
     * Tests setters / getters symmetry
     */
    @Test
    void testCompanyTitleDtoSettersAndGetters() {
        CompanyTitleDto dto = new CompanyTitleDto();

        UUID id = UUID.fromString("123e4567-e89b-12d3-a456-426614174000");
        Integer chamberId = 1;
        String title = "test-value";
        BigInteger chamberTitleId = new BigInteger("1");
        CompanyDto company = new CompanyDto();
        LocalDateTime dateCreated = LocalDateTime.of(2025, 1, 1, 10, 0);
        LocalDateTime lastUpdated = LocalDateTime.of(2025, 1, 1, 10, 0);
        Boolean recdeleted = Boolean.TRUE;

        dto.setId(id);
        dto.setChamberId(chamberId);
        dto.setTitle(title);
        dto.setChamberTitleId(chamberTitleId);
        dto.setCompany(company);
        dto.setDateCreated(dateCreated);
        dto.setLastUpdated(lastUpdated);
        dto.setRecdeleted(recdeleted);

        assertThat(dto.getId()).isEqualTo(id);
        assertThat(dto.getChamberId()).isEqualTo(chamberId);
        assertThat(dto.getTitle()).isEqualTo(title);
        assertThat(dto.getChamberTitleId()).isEqualTo(chamberTitleId);
        assertThat(dto.getCompany()).isEqualTo(company);
        assertThat(dto.getDateCreated()).isEqualTo(dateCreated);
        assertThat(dto.getLastUpdated()).isEqualTo(lastUpdated);
        assertThat(dto.getRecdeleted()).isEqualTo(recdeleted);
    }

}
